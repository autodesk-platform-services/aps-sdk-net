...
namespace Autodesk.ModelDerivative
{
    ...
    public class ModelDerivativeClient
    {
        ...
        public async System.Threading.Tasks.Task<SpecificProperties> FetchSpecificPropertiesAsync(string accessToken, string urn, string modelGuid, SpecificPropertiesPayload specificPropertiesPayload, Region region = default, string acceptEncoding = default, bool throwOnError = true)
        {
            var response = await this.MetadataApi.FetchSpecificPropertiesAsync(urn, modelGuid, acceptEncoding, region, specificPropertiesPayload, accessToken, throwOnError);
            if (response.HttpResponse.StatusCode == System.Net.HttpStatusCode.Accepted)
